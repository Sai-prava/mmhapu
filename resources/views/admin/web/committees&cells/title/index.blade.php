...
@section('content')
    <div class="main-body">
        <div class="page-wrapper">
            <div class="row">
                <div class="col-md-5">
                    <form class="needs-validation" action="{{ route('admin.CommitteesCellsTitle.store') }}" method="post">
                        @csrf
                        <div class="card">
                            <div class="card-header">
                                <h5>Add New Committees / Cells</h5>
                            </div>
                                                    ...
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title" id="editModalLabel{{ $data->id }}">
                                                                Edit Committees Title</h5>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                                aria-label="Close">X</button>
                                                        </div>
                                                        <form class="needs-validation" novalidate action="{{ route('admin.CommitteesCellsTitle.update', $data->id) }}"
                                                            method="POST">
                                                            @csrf
                                                            ...
                                                            <div class="modal-body">
                                                                <div class="mb-3 form-group">
                                                                    <label for="title{{ $data->id }}"
                                                                        class="form-label">Committees Title</label>
                                                                ...
                                                                <div class="mb-3">
                                                                    <label for="type{{ $data->id }}" class="form-label">Type (Committees / Cells)</label>
                                                                    <select name="type" class="form-control" id="type{{ $data->id }}" required>
                                                                        <option value="committees" {{ $data->type == 'committees' ? 'selected' : '' }}>Committees</option>
                                                                        <option value="cells" {{ $data->type == 'cells' ? 'selected' : '' }}>Cells</option>
                                                                    </select>
                                                                </div>
                                                            </div>
